...
const DropzoneContent = ({ uploadButton = false, className }: { uploadButton?: boolean; className?: string }) => {
  const {
    files,
    setFiles,
    onUpload,
  ...
  return (
    <div className={cn('flex flex-col', className)}>
          ...
          >
            {file.type.startsWith('image/') ? (
              <div className="h-10 w-10 rounded border overflow-hidden shrink-0 bg-muted flex items-center justify-center">
                {file.preview && <Image src={file.preview} alt={file.name} className="object-cover" />}
              </div>
            ) : (
      ...
      {uploadButton && files.length > 0 && !exceedMaxFiles && (
        <div className="mt-2">
          <Button
            variant="outline"
            onClick={onUpload}
